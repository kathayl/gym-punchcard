addEventListener('fetch', event => {
  ...
  async function handleRequest(request) {
	...
	try {
	  if (path[0] === 'api' && path[1] === 'punch' && request.method === 'POST') {
		response = await handlePunch(request);
	  } else if (path[0] === 'api' && path[1] === 'status' && request.method === 'GET') {
		response = await handleStatus(request);
	  } else if (path[0] === 'api' && path[1] === 'reward' && request.method === 'POST') {
		response = await handleReward(request);
	  } else if (path[0] === 'api' && path[1] === 'history' && request.method === 'GET') {
		response = await handleHistory(request);
	  } else if (path[0] === 'api' && path[1] === 'delete' && request.method === 'POST') {
		response = await handleDelete(request);
	  } else if (path[0] === 'api' && path[1] === 'edit' && request.method === 'POST') {
